...
def create_order(event, context):
    ...
    try:
        ...
        converted_body = convert_floats_to_decimal(body)
        
        # Prepare order data
        order_data = {
            'orderId': order_id,
            'timestamp': timestamp,
            'customerId': converted_body['customerId'],
            'items': converted_body['items'],
            'totalAmount': converted_body['totalAmount'],
            'status': 'PENDING',
            'createdAt': timestamp,
            'updatedAt': timestamp
        }
        ...
        # Add optional fields
        if 'customerEmail' in converted_body:
            order_data['customerEmail'] = converted_body['customerEmail']
        if 'shippingAddress' in converted_body:
            order_data['shippingAddress'] = converted_body['shippingAddress']
        if 'billingAddress' in converted_body:
            order_data['billingAddress'] = converted_body['billingAddress']
        ...
        if ORDER_QUEUE_URL.endswith('.fifo'):
            sqs_params['MessageGroupId'] = order_id
